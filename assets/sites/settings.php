...
if (getenv('LAGOON')){
  ...
  //NOTE: If your site requires redis - uncomment the code below
  ...
  try {
    ...
    if(!file_exists('sites/all/modules/contrib/redis')) {
      throw new \Exception('Redis is not installed - skipping');
    }

    $conf['redis_client_interface']      = 'PhpRedis';
    $conf['redis_client_host']           = $redis_host;  // Your Redis instance hostname.
    ...
    $conf['redis_client_host'] = null;
  }
  // Hash Salt
  $drupal_hash_salt = hash('sha256', getenv('LAGOON_PROJECT'));

  // Fastly setup
  $conf['fastly_api_key'] = getenv('FASTLY_API_TOKEN') ?: '';
  $conf['fastly_service_id'] = getenv('FASTLY_API_SERVICE') ?: '';
  $conf['fastly_purge_type'] = getenv('FASTLY_PURGE_TYPE') ?: 'soft';

  // Expire module setup

  $conf['expire_status'] = 2;//"EXPIRE_STATUS_ENABLED_EXTERNAL" is 2
  //BY DEFAULT WE USE EXPIRE_NODE_INSERT, EXPIRE_NODE_UPDATE and EXPIRE_NODE_DELETE
  $conf['expire_node_actions'] = [1, 2, 3];
  //WE EXPIRE ALL FILE ACTIONS
  $conf['expire_file_actions'] = [1, 2];
  //WE EXPIRE ALL COMMENT ACTIONS
  $conf['expire_comment_actions'] = [1, 2, 3, 4, 5];
